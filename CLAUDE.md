...
- `frontend/` — Next.js (App Router, TypeScript). Presentation only: five pages, split into a marketing shell and an app shell via route groups — `app/(marketing)/page.tsx`, `app/(marketing)/about/`, `app/(app)/profile/`, `app/(app)/commitments/`, `app/(app)/dashboard/`, `app/(app)/simulator/` — plus `lib/format.ts` (the sen→ringgit conversion, now the real presentation layer — see "The unit rule" below), `lib/api.ts` (FastAPI client).
